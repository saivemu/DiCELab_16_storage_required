%% Parametric Study of Storage vs Cut-off frequency vs Reliability
...
%% Parametric Study - Storage vs Tlow (Wlow1_min) aka Cut-off frequency vs Reliability
    ...
    [b1,a1] = butter(2,w_norm_low1,'low'); % butterworth filter
    ...
    Netload(isnan(Netload))= nanmean(Netload); % replace blank/missing data from excel sheet with mean of the data
    ...
    StorageSignal = Netload-lowpass_signal;
    
     z = gt(max(abs(StorageSignal)),1000) ;  % To remove the anamolies/outliers 
       ...
       Pbattery(n,1) = max(abs(StorageSignal));  % Maximum storage required in a day
        
        end
        
        if max(abs(cumtrapz(t,StorageSignal)))/4000   == 0 
         Ebattery(n,1) = NaN;
        else
         Ebattery(n,1) = max(abs(cumtrapz(t,StorageSignal)))/3600;
        end
...
%         k = ((wlow-wlow1)*(10^5))+1;
    ...
    % Kernel Density Estimation for Power Capacity (MW)
    row = 1;
    ...
    % Kernel Density Estimation for Energy Capacity (MW)
    ...
    % Power Storage vs Reliability vs Cut-off Frequency Surface plot
    figure('Name','Pbatt Surf')
    surf(relreq',surfaxis',Pbatt)
    xlabel('Reliability')
    ylabel('Frequency (rad/s)')
    zlabel('Pbatt(MW)')
    % Energy Storage vs Reliability vs Cut-off Frequency Surface plot
